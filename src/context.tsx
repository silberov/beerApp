...
interface ContextState {
  ...
  loading: boolean;
  error: string;
  setSelectedId: (beerId: string) => void;
}

const ApiCtx = createContext<ContextState>({} as ContextState);

const defaultbeers: beerType[] = [];

export function ApiCtxProvider({ children }: any) {
  // Initialize state
  const [beers, setBeers]: [beerType[], (beer: beerType[]) => void] =
    useState(defaultbeers);

  const [loading, setLoading]: [boolean, (loading: boolean) => void] =
    useState<boolean>(false);

  const [selectedId, setSelectedId]: [string, (beerId: string) => void] =
    useState('');
    // fix that any
  ...
  useEffect(() => {
    axios
      .get<beerType[]>(`https://api.punkapi.com/v2/beers/${selectedId}`)
      .then((response) => {
  ...
  return (
    <ApiCtx.Provider value={{ beers, loading, error, setSelectedId }}>
      {children}
    </ApiCtx.Provider>
  );
}

export default ApiCtxProvider;
